...
classroom_width, classroom_length = 7, 5
num_app = 5
population_size = 12
generations = 10
mutation_rate =0.1
...
def visualize_initial_and_fina(classroom_width, classroom_length, initial_population, final_population):
    plt.figure(figsize=(15, 6))

    for i, (x, y) in enumerate(initial_population):
        plt.text(i, y, str(i + 1), color='black', fontsize=8, ha='center', va='center')
    ...
    for i, (x, y) in enumerate(final_population):
        plt.text(i, y, str(i + 1), color='black', fontsize=8, ha='center', va='center')
    plt.scatter(*zip(*final_population), color='red', marker='o',
...
for generation in range(generations):
    ...
    select_parents = random.choices(population[generation], weights=fitness_scores, k=population_size)

    offspring = []
    for _ in range(population_size):
        parent_1, parent_2 = random.sample(select_parents, 2)
        crossover_x = random.uniform(parent_1[0], parent_2[0])
        crossover_y = random.uniform(parent_1[1], parent_2[1])
